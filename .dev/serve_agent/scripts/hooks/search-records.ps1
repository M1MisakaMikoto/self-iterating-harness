<#
.SYNOPSIS
    path-memory 检索：按 tag / 关键词在 .dev/serve_project/records 中搜索记录。
.EXAMPLE
...
function Find-RecordsDir {
    ...
    $records = Join-Path $root '.dev\serve_project\records'
...
if (-not $recordsDir) { Write-Output '未找到 records 目录（.dev/serve_project/records）'; exit 1 }
